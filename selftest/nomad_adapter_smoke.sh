#!/usr/bin/env bash
# selftest/nomad_adapter_smoke.sh
#
# Phase G+2 (2026-05-13): dedicated top-level selftest gate for the
# NOMAD (NOvel MAterials Discovery) adapter.
#
...
#   dedicated gate runs the NOMAD adapter DIRECTLY so that a NOMAD-only
#   regression is identifiable in selftest output. Mirrors the discipline
#   of the COD adapter gate.
#
# Behaviour:
#   - Invokes: python3 _absorption_bridge/nomad/nomad_search_smoke.py --selftest
#   - PASS  : exit 0 AND stdout contains "__HEXA_MATTER_NOMAD_SEARCH_SMOKE__ PASS"
...
# nomad-lab.eu occur in CI.
#
# Sentinel emitted:
#   __HEXA_MATTER_NOMAD_ADAPTER_SMOKE__ PASS / FAIL

set -u

HERE="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="${HEXA_MATTER_ROOT:-$(cd "$HERE/.." && pwd)}"
ADAPTER="$REPO_ROOT/_absorption_bridge/nomad/nomad_search_smoke.py"
SENTINEL="__HEXA_MATTER_NOMAD_ADAPTER_SMOKE__"
ADAPTER_SENTINEL="__HEXA_MATTER_NOMAD_SEARCH_SMOKE__ PASS"

echo "hexa-matter/selftest/nomad_adapter_smoke — NOMAD (offline)"
