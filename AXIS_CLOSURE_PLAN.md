...
>
> **honest C3 (raw#10)**: this file enumerates per-group closure *roadmap* —
...
- **(c) wet-lab synthesis / manufacturing scale closure** — physical fabrication, vendor procurement, regulatory pathway. **OUT-OF-REPO** by definition. The software side is ready (vendor numbers vendored, no lattice fit per raw#10 C3); closure depends on external counterparty selection.
...
- **(b)** queued for Phase B/F — none of these are v1.x blockers. They are Phase F (research bridge) targets, tracked here for visibility. **Phase B IMPLEMENTED 2026-05-13**: `selftest/run_all.sh` runs 23 fine-grained gates (8 cross-cutting + 8 group-specific + 4 verb-specific + 3 bridge aggregators) covering registry consistency, regression, NIST anchor presence, lattice-fit-on-external-entities enforcement (raw#10 C3), CER thermal-shock, POL T_g, FIB tensile, MET alloy classification, GEM authenticity, PRC yield honesty, FAS dye chemistry, silicon purity, compound-semi bandgap, magnetic Curie+BHmax, CNT 80 GPa honesty caveat, MOF DAC $100/t UNPROVEN preservation. `__HEXA_MATTER_SELFTEST__ PASS  (23/23)`. **Phase E IMPLEMENTED 2026-05-13**: `_python_bridge/` ships 12 scientific-compute modules (6 stdlib FUNCTIONAL + 2 RDKit PARTIAL + 2 ASE PARTIAL + 2 pymatgen PARTIAL); each module has `--selftest` mode that runs offline/deterministically and SKIPs cleanly when optional deps (RDKit/ASE/pymatgen) are missing. Bridge aggregator: `selftest/pyproject_smoke.sh` → `__HEXA_MATTER_PYTHON_BRIDGE__ PASS (12/12 modules, 5 skipped)` in stock-Python env. **Phase F IMPLEMENTED 2026-05-13**: `_research_bridge/` ships 8 absorption modules (arxiv 2 + web 3 + selftest 3) for arxiv deep research + web deep research (vendor datasheets, materials industry RSS/Atom, USPTO/EPO patent search); each module has `--selftest` offline-replay mode (fixtures only — NO live network calls in selftest); live mode (`--live`) is gated and rate-limit aware (arxiv 3-sec backoff, vendor robots.txt). Bridge aggregator: `selftest/research_bridge_smoke.sh` → `__HEXA_MATTER_RESEARCH_BRIDGE__ PASS (3/3 modules, 0 skipped)` (stdlib path); raw#10 C3 preserved (no n=6 lattice-fit on ingested data); UNPROVEN flags (LK-99, $100/t MOF, 25-year perovskite) propagate through verb-keyword digest. **Phase G IMPLEMENTED 2026-05-13**: `_absorption_bridge/` ships 10 external-system adapters (5 databases/APIs + 5 universal force fields) for AlphaFold-class materials-discovery absorption per user directive ("알파폴드 처럼 흡수할 시스템도 흡수"): Materials Project (Berkeley/LBNL ~154k materials, mp-api SDK) · GNoME (DeepMind 2.2M predicted-stable crystals, Zenodo DOI 10.5281/zenodo.10371563, **PREDICTED NOT SYNTHESIZED** marker preserved per INIT.md hard rule 5) · Matlantis (Preferred Networks PFP universal NNP, **COMMERCIAL UNVERIFIED at hexa-matter scale economics**) · OMat24 (Meta AI 110M structures + MACE-OMat checkpoint, HuggingFace `fairchem/OMAT24`) · SchNet (Schütt 2017) · MACE (Batatia 2022) · ALIGNN (Choudhary 2021) · CHGNet (Deng 2023) · M3GNet via MatGL (Chen 2022). Each adapter has `--selftest` offline fixture-replay mode (md5-stamped JSON in `<system>/cache/`); live API path SKIPs by default per "NO LIVE API CALLS in selftest" rule. Bridge aggregator: `selftest/absorption_bridge_smoke.sh` → `__HEXA_MATTER_ABSORPTION_BRIDGE__ PASS (6/6 modules, 0 skipped)` (stdlib path); raw#10 C3 preserved (no n=6 lattice-fit on MP/GNoME/Matlantis/OMat24/FF outputs — each external system carries its OWN published error bars: DFT-PBE typical, NNP force MAE 20–60 meV/Å).
